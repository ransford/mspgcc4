...
static void msp430_function_prologue (FILE * file, HOST_WIDE_INT);
static void msp430_function_epilogue (FILE * file, HOST_WIDE_INT);
...
static rtx msp430_expand_builtin (tree, rtx, rtx, enum machine_mode, int);
...
static int
msp430_save_prologue_function_p (
     tree func)
{
  tree a;
  if (TREE_CODE (func) != FUNCTION_DECL)
...
static void msp430_function_prologue (FILE *file, HOST_WIDE_INT size)
{
  int i;
  int interrupt_func_p = interrupt_function_p (current_function_decl);
  ...
  if (signal_func_p && !interrupt_func_p)
    {
      warning (OPT_Wattributes, "attribute 'signal' does not make sense.");
      signal_func_p = 0;
    }
...
static void msp430_function_epilogue (FILE *file, HOST_WIDE_INT size)
{
  int i;
  int interrupt_func_p = interrupt_function_p (current_function_decl);
  int leaf_func_p = leaf_function_p ();
...
msp430_init_builtins (void)
{
  ...
  add_builtin_function (msp430builtins[0].name, 
		    build_function_type (void_type_node, args),
		    0, BUILT_IN_MD, NULL, NULL_TREE);
  
  add_builtin_function (msp430builtins[1].name, 
		    build_function_type (void_type_node, args),
		    1, BUILT_IN_MD, NULL, NULL_TREE);

  args = tree_cons (NULL_TREE, void_type_node, void_list_node);
  add_builtin_function (msp430builtins[2].name, 
		    build_function_type (ptr_type_node, args),
		    2, BUILT_IN_MD, NULL, NULL_TREE);
